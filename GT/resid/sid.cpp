...
// Set chip model.
// ----------------------------------------------------------------------------
void SIDC64::set_chip_model(chip_model model)
{
  for (int i = 0; i < 3; i++) {
...
// Write registers.
// ----------------------------------------------------------------------------
void SIDC64::write(reg8 offset, reg8 value)
{
  bus_value = value;
...
// Read state.
// ----------------------------------------------------------------------------
SIDC64::State SIDC64::read_state()
{
  State state;
  int i, j;
...
// Write state.
// ----------------------------------------------------------------------------
void SIDC64::write_state(const State& state)
{
  int i;
...
// Enable filter.
// ----------------------------------------------------------------------------
void SIDC64::enable_filter(bool enable)
{
...
// ----------------------------------------------------------------------------
void SIDC64::enable_external_filter(bool enable)
{
...
// not overfilled.
// ----------------------------------------------------------------------------
bool SIDC64::set_sampling_parameters(double clock_freq, sampling_method method,
          ...
          double filter_scale)
{
  ...
  int n = (int)ceil(log((double)(res/f_cycles_per_sample))/log(2.0));
  fir_RES = 1 << n;
...
// ----------------------------------------------------------------------------
void SIDC64::adjust_sampling_frequency(double sample_freq)
{
  cycles_per_sample =
...
// ----------------------------------------------------------------------------
void SIDC64::fc_default(const fc_point*& points, int& count)
{
...
// ----------------------------------------------------------------------------
PointPlotter<sound_sample> SIDC64::fc_plotter()
{
...
// ----------------------------------------------------------------------------
void SIDC64::clock(cycle_count delta_t)
{
  int i;
...
// 
// ----------------------------------------------------------------------------
int SIDC64::clock(cycle_count& delta_t, short* buf, int n, int interleave)
{
  switch (sampling) {
  default:
  case SAMPLE_FAST:
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDC64::clock_fast(cycle_count& delta_t, short* buf, int n,
        int interleave)
{
  int s = 0;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDC64::clock_interpolate(cycle_count& delta_t, short* buf, int n,
         int interleave)
{
  int s = 0;
  int i;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDC64::clock_resample_interpolate(cycle_count& delta_t, short* buf, int n,
            int interleave)
{
  int s = 0;
...
// ----------------------------------------------------------------------------
RESID_INLINE
int SIDC64::clock_resample_fast(cycle_count& delta_t, short* buf, int n,
           int interleave)
{
  int s = 0;
